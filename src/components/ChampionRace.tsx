...
export default function ChampionRace({
  ...
  return (
    <section className="mb-8 rounded-2xl border border-white/10 bg-white/[0.03] p-5">
      ...
      <p className="mb-4 mt-0.5 text-xs text-white/40">
        {total > 0
          ...
          {top3.map((r, i) => {
            ...
            return (
              <div key={r.team} className="flex items-center gap-3">
                ...
                <span className="w-14 shrink-0 text-right text-[11px] text-white/40">
            ...
            <div className="mt-3 space-y-1.5 border-t border-white/10 pt-3">
              {rest.map((r, i) => {
                ...
                return (
                  ...
                  >
                    <span className="w-6 shrink-0 text-center text-white/35">
                      {i + 4}
                    </span>
                    {t && <Flag code={t.code} size="sm" />}
                    <span className="flex-1 truncate">{r.team}</span>
                    <span className="tabular-nums text-white/50">{r.pct}%</span>
